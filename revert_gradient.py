...
content = content.replace(new_bGradNormal, old_bGradNormal)
...
old_bGradFlipped = "`linear-gradient(270deg, #fdf1e7 0%, #fed3ab 64%, rgba(254, 211, 171, 0) 66%), radial-gradient(circle at 100% 50%, #ffffff 0%, #ffffff 14%, #fbf0e8 15%, #fbf0e8 18%, #fce9d8 19%, #fce9d8 22%, #fde4c8 23%, #fde4c8 26%, #fed3ab 27%, #fed3ab 100%)`"
content = content.replace(new_bGradFlipped, old_bGradFlipped)
...
content = content.replace(new_wGradNormal, old_wGradNormal)
...
content = content.replace(new_wGradFlipped, old_wGradFlipped)

with open(js_file, 'w') as f:
    f.write(content)
print("Reverted JS gradients")
