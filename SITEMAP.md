...
| **ui** | `feat` | Right-Panel Shadow Rebalanced to Study Mode: the main app `.right-panel` drops its box-shadow (now matches the left panels), and the full shadow (`-8px 0 24px`, `-2px 0 8px`, `0 10px 30px -10px`) moves onto the study modal — applied to the inner `.study-max-boundary-box` board card (the outer `.study-modal-left` could not show a shadow edge-to-edge: it fills the modal, and both it and `#study-modal` clip via `overflow: hidden`), and the smaller shadow on `.study-modal-right` is removed, so the shadow shows only inside study mode. |
