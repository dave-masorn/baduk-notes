...
### v0.1.057 — Replacing Dead Stones no longer removes the dead stones from SGF Properties: DD/MA counts stay intact while the prisoner fill drops the score by exactly 1 per player
...
- **Marks survive replacing (v0.1.057).** Filling any territory point other than a dead-marked cell pops the dead bucket for placement but leaves `markedDead`/`deadStonesInfo` untouched — the prisoner term in the score drops by decrementing the capture counter instead, so the `DD`/`MA` counts in SGF Properties never shrink while Replacing Dead Stones and the margin stays pinned at exactly −1 per player.
