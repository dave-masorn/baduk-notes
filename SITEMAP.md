...
### Set C — Hamaguri & Slate (Material Renderer)

Set C renders each stone from its actual material with a pure Canvas 2D renderer (no image assets), modeled on real stones rather than a generic gradient:

| Stone | Material | Signature |
|-------|----------|-----------|
| Black | Slate (那智黒 nachiguro) | Matte-glossy near-black with a subtle blue-green mineral tint; sparse, faint, roughly-parallel grain streaks; larger but dimmer/softer highlight than glass |
| White | Hamaguri (蛤 clam-shell) | Ivory/cream base with a warm translucent amber band near the rim; concentric growth-ring lines radiating from an off-center "hinge" origin, like a fingerprint |

The renderer is self-contained and additive (`annotation_v4.js:5940-6251`): `_getHamaguriTexture()` / `_getSlateTexture()` build the textures once per (radius, variant) into an off-screen canvas cache (`_stoneTextureCache`), `drawGoStone()` paints the stone with its own drop shadow, material texture, warm edge glow (white), and a specular highlight, and `getStoneVariant()` derives per-stone texture params from board position.

**Per-position, not per-placement.** Texture params are derived deterministically from `(row * 19 + col)` with a per-player seed offset, so a stone's grain is stable across every redraw (hover, undo, resize, export preview — no flicker) while each position still looks distinct from its neighbors. Capturing and replaying a stone on the same point keeps the same pattern. Specular is hard-capped at 0–0.5 (50%) to keep the finish matte-glossy rather than glassy.

**Code** (`annotation_v4.js:6687-6695`) — a new leading branch in the stone-surface chain; Set A/B, image, and solid-color paths are untouched:
