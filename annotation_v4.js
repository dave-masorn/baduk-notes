...
const SFX_BASE64 = {
    ...
    stoneStudy: 'data:audio/wav;base64,',
...
function recordMoveAt(r, c, color, toolName) {
    ...
    const captureCheck = playStoneWithCaptures(tempBoard, r, c, color);
    ...
    state.popupShownForCurrentChange = false;
    // A move that captures ("take") plays the capture SFX once instead of the
...
// ============================================================
// Study Record storage engines now live in study-storage.js
// (StudyRecordDB, StudyDirStore, formatStudyAccessTime, dir setup UI)
// ============================================================
...
// ============================================================
// Canvas tools + board rendering pipeline now live in board-renderer.js
// (getGoTerm..drawCenteredRichText: canvas tools, board renderer,
//  stone/texture lib, cell content, rich text)
// ============================================================


// ============================================================
// Export modal / diagram generation now live in export-diagram.js
// (loadRecoloredSvg..updateLegendUI)
// ============================================================
...
function initFloatingToolbar() {
    ...
    document.querySelectorAll('.custom-stones-header').forEach(header => {
        header.addEventListener('click', () => {
            ...
            if (!section) return;
            setCustomStonesExpanded(section, !section.classList.contains('expanded'));
        });
    });
...
function setCustomStonesExpanded(section, expanded) {
    if (!section) return;
    ...
    if (expanded) {
        ...
        // active the custom body starts collapsed, so an accordion opened in that state measures
        // SHORT — expanding Custom Stones then clips its growth behind the accordion's
        // overflow:hidden, so it appears to never expand. Re-fit the enclosing accordion to its
        // final content height in the same frame the body expands.
        const accordion = section.closest('.accordion-content');
        const accordionOpen = !!(accordion && accordion.classList.contains('open'));
        // Measure the final heights while the body is at natural size — a measurement during the
        // max-height transition would capture the still-clipped starting height.
...
function syncCustomStonesSection() {
    ...
    // The Custom Stones controls are available for every Default Stone Set (A/B/C) — when a set
    // is selected the section auto-collapses but stays expandable, and it auto-expands when no
    // set is active.
    setCustomStonesExpanded(section, !hasActive);
}
...
function saveStyleAndRedraw() {
    ...
    // Keep the study viewport's backdrop in sync with the BG color whenever the
    // editor updates the style (drawBoard() only repaints the canvas, not the
    // padded area of the viewport around it).
    if (document.body.classList.contains('study-mode-active')) {
...
// ============================================================
// Manual scoring modal + board renderer now live in scoring.js
// (window.scoringState, scoringHistory/Future, initScoringModal..
//  renderScoringBoardToCtx, _deferredScoringInit)
// ============================================================
